...
class _DetailScreenState extends State<DetailScreen> {
  ...
  void updatePrice(String size, String option) {
    setState(() {
      ...
      int basePrice = widget.product.price.toInt(); 
      int sizeMarkup = 0;
  ...
  Widget _buildProductImage(String imageSource) {
    if (imageSource.length > 100) {
      try {
        return Image.memory(base64Decode(imageSource), fit: BoxFit.cover, width: double.infinity, height: 450);
      } catch (e) {
        return const SizedBox(height: 450, child: Icon(Icons.broken_image));
      }
    } else if (imageSource.toLowerCase().endsWith('.svg')) {
      return SvgPicture.asset(imageSource, fit: BoxFit.cover, width: double.infinity, height: 450);
    } else if (imageSource.startsWith("http")) {
      return Image.network(imageSource, fit: BoxFit.cover, width: double.infinity, height: 450);
    } else {
      return Image.asset(imageSource, fit: BoxFit.cover, width: double.infinity, height: 450);
    }
  }
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(color: badgeBgColor, borderRadius: BorderRadius.circular(30)),
                            child: Text("Sisa ${widget.product.stock} stok", style: TextStyle(color: badgeTextColor, fontWeight: FontWeight.bold, fontSize: 12)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 15),
                      Text(formatIDR(currentPrice), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF7E8959))),
                      const SizedBox(height: 25),
                      const Text("Tentang Jamu Ini", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 10),
                      Text(
                        widget.product.description.isEmpty ? "Racikan jamu tradisional ini dibuat dengan bahan alami pilihan tanpa pengawet." : widget.product.description,
