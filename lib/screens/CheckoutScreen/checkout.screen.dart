...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    super.initState();
    // Mengonversi CartItem ke Map agar mudah dikelola di UI ini
                ...
                children: [
                  ...
                  _buildClickableSection(
                    ...
                    onTap: () async {
                      ...
                      if (result != null) setState(() => selectedPayment = result);
                    },
                  ),
                  const Divider(thickness: 8, color: Color(0xFFF1F1F1)),
                ],
  ...
  Widget _buildDeliveryMode() => Container(
        ...
        child: Row(
          children: [
            ...
            Expanded( // Mencegah Overflow teks "Pick Up"
              child: Column(
  ...
  Widget _buildLocationInfo() => Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
      ...
      child: Row(
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: (item['image'] != null && item['image'].toString().startsWith('assets'))
                ? Image.asset(
                    item['image'],
                    width: 60,
                    height: 60,
          ...
          // FIX OVERFLOW: Menggunakan Expanded agar Column tidak menabrak pinggir
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                  ...
                  maxLines: 2, // Biar kalau panjang tidak berantakan
                  overflow: TextOverflow.ellipsis, // Pakai titik-titik kalau kepanjangan
                ),
                Text(
                  item['size'] ?? '350 ml',
                  style: const TextStyle(
  ...
  Widget _buildRincianBayar() => Padding(
        padding: const EdgeInsets.all(16),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
        ...
        onPressed: () async {
          try {
            ...
            final newOrder = OrderModel(
              ...
              paymentMethod: selectedPayment != null ? selectedPayment!['name'] : 'Tunai',
              address: selectedBooth,
              createdAt: DateTime.now(),
              // FIX ERROR INVALID ARGUMENT: Membersihkan data agar Firestore tidak error
              items: localCartItems.map((item) => {
                'name': item['name'],
                'price': item['price'],
                'qty': item['qty'],
                'size': item['size'],
                'image': (item['image'] is String && !item['image'].startsWith('iVBO')) 
                          ? item['image'] 
                          : 'assets/images/logo.png', // Ganti dengan path logo kamu
              }).toList(),
              image: localCartItems.isNotEmpty ? localCartItems.first['image'] : '',
