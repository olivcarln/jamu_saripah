...
class OrderMethod extends StatelessWidget {
  ...
  const OrderMethod({
    ...
    // userName dihapus dari constructor agar mengambil data otomatis dari Firestore
  });

  @override
  Widget build(BuildContext context) {
    // 🔥 LOGIC SYNC: Mengambil UID user yang sedang login
    final String? uid = FirebaseAuth.instance.currentUser?.uid;

    return StreamBuilder<DocumentSnapshot>(
      // Mengambil data dari koleksi 'users' sesuai struktur yang kamu punya di Firebase
      stream: FirebaseFirestore.instance.collection('users').doc(uid).snapshots(),
      builder: (context, snapshot) {
        // Fallback jika data belum ada atau sedang loading
        ...
        // --- UI KAMU DI BAWAH INI TIDAK BERUBAH SAMA SEKALI ---
        return Padding(
              ...
              Padding(
                ...
                child: Text.rich(
                  TextSpan(
                    ...
                    children: [
                      TextSpan(
                        text: currentName, // ✅ Disinkronkan dengan Firestore
                        style: const TextStyle(
                          color: Color(0xFF5E6D45), // ✅ warna ijo tetap
                        ),
                      ),
                      const TextSpan(
